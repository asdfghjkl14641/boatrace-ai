...
import os as _os
TRAIN_START = _os.environ.get("V4_TRAIN_FROM", "2023-05-01")
TRAIN_END   = _os.environ.get("V4_TRAIN_TO",   "2025-12-31")
TEST_START  = _os.environ.get("V4_TEST_FROM",  "2026-01-01")
TEST_END    = _os.environ.get("V4_TEST_TO",    "2026-04-18")
...
pd.DataFrame({
    ...
    "beta_old_H": beta_old,
    "beta_new_H": beta_new,
}).to_csv(OUTD / f"v3{OUT_SUFFIX}_weights.csv", index=False, encoding="utf-8-sig")

pd.DataFrame([
    {"model":"v2_old (旧train旧H)", **V2_OLD},
    {"model":"v3_oldH_redo",  "tau":best_old["tau"], "hit1":best_old["hit1"], "hit2":best_old["hit2"], "hit3":best_old["hit3"], "ll":best_old["ll"], "pl_ll":best_old["pl_ll"], "train_races":len(keys_train), "train_period": f"{TRAIN_START}〜{TRAIN_END}"},
    {"model":"v3_newH",  "tau":best_new["tau"], "hit1":best_new["hit1"], "hit2":best_new["hit2"], "hit3":best_new["hit3"], "ll":best_new["ll"], "pl_ll":best_new["pl_ll"], "train_races":len(keys_train), "train_period": f"{TRAIN_START}〜{TRAIN_END}"},
]).to_csv(OUTD / f"v3{OUT_SUFFIX}_three_versions_comparison.csv", index=False, encoding="utf-8-sig")

print("\nCSV 書込: v3_weights.csv, v3_three_versions_comparison.csv")
print("完了")
